import { Accessor, createMemo, createSignal, createEffect, batch, Setter } from "./deps.ts"
...
import { math_max, math_min, math_abs, math_sin, math_cos } from "./deps.ts"
...
class Grid implements NonNullable<GridInit> {
	...
	private colWidths: Accessor<number[]>
	private rowHeights: Accessor<number[]>
	public cellFrames: Accessor<CellFrameInfo[][]>
	...
	constructor(config: GridInit) {
		...
		this.cellFrames = createMemo<CellFrameInfo[][]>(() => {
			console.log("recomputing cellFrames")
			const
				{ rows, cols, cells, colGap, rowGap, colAlign: colAlignGlobal, rowAlign: rowAlignGlobal } = this,
	...
	cellFrame = (row: number, col: number): CellFrameInfo => {
		const { rows, cols, cellFrames } = this
		return cellFrames()[row % rows][col % cols]
	}
