import { cumulativeSum, math_abs, math_cos, math_sin, number_isFinite } from "./deps.js";
...
*/
export const alignmentToNumber = (alignment, reverse = false) => {
...
*/
export const parseAlignments = (alignments, reverse = false) => {
...
const parseLengthUnitLiteral = (str) => {
...
export const parseLengthUnit = (length) => {
...
const length_unit_name_iter = ["px", "pw", "ph"];
export const stringifyLengthUnit = (length) => {
    const length_unit = parseLengthUnit(length);
    return length_unit_name_iter.map((unit_name) => (String(length_unit[unit_name] ?? 0) + unit_name)).join(" + ");
};
