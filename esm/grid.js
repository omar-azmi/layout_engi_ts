 ...
 * @module
*/
import { Array2DShape, DEBUG, clamp, cumulativeSum, math_max, newArray2D, number_POSITIVE_INFINITY, rotateArray2DMajor, rotateArray2DMinor, spliceArray2DMajor, spliceArray2DMinor } from "./deps.js";
import { alignmentToNumber, boundboxOfRotatedRect, parseAlignments } from "./funcdefs.js";
import { SignalingClass } from "./signal.js";
/** the grid layout class provides a way for you to compute the locations of sprites had they been organized in a grid. */
export class Grid extends SignalingClass {
    cols;
    rows;
    originAlign;
    colWidth;
    ...
    getColWidths = this.comp((id) => {
        ...
        for (let c = 0; c < cols; c++) {
            ...
            max_widths[c] = math_max(...this.getCol(c).map((cell) => {
                const { width: sprite_width, height: sprite_height = 0, rotation } = cell, width = sprite_width !== undefined ?
    ...
    getRowHeights = this.comp((id) => {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            max_heights[r] = math_max(...this.getRow(r).map((cell) => {
                const { width: sprite_width = 0, height: sprite_height, rotation } = cell, height = sprite_height !== undefined ?
    ...
    */
    get_topleft_aligned_cell_frames = this.comp((id) => {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const left = left_vals[c], top = top_vals[r], thisColWidth = colWidths[c], thisRowHeight = rowHeights[r], right = left + thisColWidth, bottom = top + thisRowHeight, { width = 0, height = 0, rotation, anchor: { x: anchorSelfX, y: anchorSelfY } = {}, colAlign = colAlignGlobal[c % colAlignGlobal_len], rowAlign = rowAlignGlobal[r % rowAlignGlobal_len], } = cells[r][c], colAlign_val = alignmentToNumber(colAlign), rowAlign_val = alignmentToNumber(rowAlign), anchorX = anchorSelfX ?? colAlign_val, anchorY = anchorSelfY ?? rowAlign_val, x = thisColWidth * colAlign_val - width * anchorX, y = thisRowHeight * rowAlign_val - height * anchorY;
                cell_frames[r][c] = {
    ...
    */
    ...
    constructor(config, lazy = false) {
        super(lazy);
        const { rows, cols, originAlign = "", colWidth = [0], rowHeight = [0], colMinWidth = [0], rowMinHeight = [0], colMaxWidth = [number_POSITIVE_INFINITY], rowMaxHeight = [number_POSITIVE_INFINITY], colAlign = ["start"], rowAlign = ["start"], colGap = [0], rowGap = [0] } = config, cells = newArray2D(rows, cols);
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                cells[r][c] = {};
            }
    ...
    getRow(row) { return this.cells[row]; }
    getCol(col) { return this.cells.map((row) => row[col]); }
